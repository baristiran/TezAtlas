...
STATUS_PHASE2 = textwrap.dedent("""\
    ...
    next_actions:
      ...
      - Add arguments to ARGUMANLAR.md
    blockers: []
    gates: {{}}
    ```
...
def tmp_project_phase3_gated(tmp_path: Path) -> Path:
    ...
    # ARGUMANLAR.md with 5 rows
    argumanlar = "# ARGUMANLAR\n\n| İddia | Kaynak |\n|-------|--------|\n"
    for i in range(5):
        argumanlar += f"| Argüman {i+1} | Source {i+1} |\n"
    (tmp_path / "ARGUMANLAR.md").write_text(argumanlar, encoding="utf-8")
...
class TestSessionLoad:
    ...
    def test_load_argumanlar_count(self, tmp_project: Path) -> None:
        argumanlar = "# ARGUMANLAR\n\n| İddia | Kaynak |\n|-------|--------|\n| A1 | S1 |\n| A2 | S2 |\n"
        (tmp_project / "ARGUMANLAR.md").write_text(argumanlar, encoding="utf-8")
        sess = TezAtlasSession(tmp_project)
        state = sess.load()
