...
notes/*.md dosyalarını ARGUMANLAR.md ile birleştirerek SENTEZ.md üretir.
...
def write_synthesis_report(
    ...
    lines = [
        ...
        "> 4. SENTEZ paragrafını yaz: 'Bu kaynaklar bütünüyle şunu gösteriyor...'",
        "",
        "---",
        "",
    ]
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", metavar="FILE", default="SENTEZ.md",
        help="Çıktı dosyası (varsayılan: SENTEZ.md)",
    )
    parser.add_argument(
    ...
    argumanlar_path = project_dir / "ARGUMANLAR.md"

    # Load arguments
    arguments = parse_argumanlar(argumanlar_path)
    if not arguments:
        print("⚠️  ARGUMANLAR.md bulunamadı ya da argüman yok.")
        print("💡 Önce ARGUMANLAR.md'e argümanlarını ekle.")
        sys.exit(0)
