...
Kontrol ettiği dosyalar:
    ...
    - ARGUMANLAR.md       → mevcut argümanlar (konu yönelimi için)
...
def extract_current_focus(project_dir: Path) -> dict:
    ...
    return {
        "source": "ARGUMANLAR.md + son notlar",
        "text": full_text[:1000],
...
def write_report(project_dir: Path, original: dict, current: dict, drift: dict) -> Path:
    ...
    status_labels = {
        ...
        "unknown": "⚪ Bilinmiyor — konu_kesfi.md veya ARGUMANLAR.md eksik",
    }
    ...
    if drift.get("status") not in ("unknown", None):
        ...
        if status in ("moderate_drift", "major_drift"):
            lines += [
                ...
                "2. **Araştırma sorusunu gözden geçir:** ARGUMANLAR.md'deki argümanlar hâlâ orijinal soruyu yanıtlıyor mu?",
...
def main() -> None:
    ...
    if not original:
        ...
        print("   Danışmanınla konuş ve ARGUMANLAR.md'i gözden geçir.")
